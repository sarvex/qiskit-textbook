...
class run_game:
    ...
    def __init__(
        ...
        def get_success(required_gates):
            ...
            for pauli in success_condition:
                success = success and (
                    abs(success_condition[pauli] - grid.rho[pauli]) < eps
                )
        ...
        if (
            allowed_gates["0"] == {}
        ):  # if no gates are allowed for qubit 0, we know to only show qubit 1
            shown_qubit = 1
        ...
        def given_gate(a):
            ...
            if gate.value in allowed_gates["both"]:
                ...
                allowed_qubits = []
                for q in ["1", "0"]:
                    if (gate.value in allowed_gates[q]) or (
                        gate.value in allowed_gates["both"]
                    ):
        ...
        def given_action(c):
            ...
            if action.value == "Apply operation":
                if qubit.value not in ["", description["qubit"][0], "Success!"]:
                    ...
                    q01 = (
                        "0" * (qubit.value == qubit_names["0"])
                        + "1" * (qubit.value == qubit_names["1"])
                        + "both" * (qubit.value == "not required")
                    )
                    if q_gate in ["bloch"]:
    ...
    def plot_spheres(self):
        return plot_bloch_multivector(
            Statevector(self.get_circuit(use_initializer=True)), reverse_bits=True
        )
...
class pauli_grid:
    ...
    def __init__(
        ...
        for pauli, value in self.box.items():
            self.points[pauli] = [
                self.ax.add_patch(
                    Circle(self.box[pauli], 0.0, color=point_color[0], zorder=10)
                )
            ]
            self.points[pauli].append(
                self.ax.add_patch(Circle(value, 0.0, color=point_color[1], zorder=10))
            )
    ...
    def update_grid(
        ...
        def add_line(line, pauli_pos, pauli):
            ...
            if unhidden:
                if line == "X":
                    a = (
                        self.box[pauli_pos][0] - length / 2,
                        self.box[pauli_pos][1] - width / 2,
                    )
                    c = (
                        self.box[pauli_pos][0] + length / 2,
                        self.box[pauli_pos][1] - width / 2,
                    ...
                    self.ax.add_patch(
                        Rectangle(
                            a, length * (1 - p), width, angle=0, color=line_color[0]
                        )
                    )
                    self.ax.add_patch(
                        Rectangle(b, length * p, width, angle=0, color=line_color[2])
                    )
                    if length * p > delta:
                        self.ax.add_patch(
                            Rectangle(
                                (b[0] + delta / 2, b[1] + delta * 0.6),
                                length * p - delta,
                                width - delta,
                                angle=0,
                                color=line_color[1],
                    ...
                    a = (
                        self.box[pauli_pos][0] - width / 2,
                        self.box[pauli_pos][1] - length / 2,
                    )
                    c = (
                        self.box[pauli_pos][0] - width / 2,
                        self.box[pauli_pos][1] + length / 2,
                    ...
                    self.ax.add_patch(
                        Rectangle(
                            a, width, length * (1 - p), angle=0, color=line_color[0]
                        )
                    )
                    self.ax.add_patch(
                        Rectangle(b, width, length * p, angle=0, color=line_color[2])
                    )
                    if length * p > delta:
                        self.ax.add_patch(
                            Rectangle(
                                (b[0] + delta / 2, b[1] + delta / 2),
                                width - delta,
                                length * p - delta,
                                angle=0,
                                color=line_color[1],
                    ...
                    a = (
                        self.box[pauli_pos][0] - length / (2 * np.sqrt(2)),
                        self.box[pauli_pos][1] - length / (2 * np.sqrt(2)),
                    )
                    c = (
                        self.box[pauli_pos][0] + length / (2 * np.sqrt(2)),
                        self.box[pauli_pos][1] + length / (2 * np.sqrt(2)),
                    ...
                    self.ax.add_patch(
                        Rectangle(
                            a, width, length * (1 - p), angle=-45, color=line_color[0]
                        )
                    )
                    self.ax.add_patch(
                        Rectangle(b, width, length * p, angle=-45, color=line_color[2])
                    )
                    if length * p > delta:
                        self.ax.add_patch(
                            Rectangle(
                                (
                                    b[0] + delta * np.sqrt(2) / 2,
                                    b[1] + delta * 0.1 * np.sqrt(2) / 2,
                                ),
                                width - delta,
                                length * p - delta,
                                angle=-45,
        ...
        if self.initial:
            for pauli in self.box:
                ...
                self.ax.add_patch(
                    Rectangle(
                        (self.box[pauli][0], self.box[pauli][1] - 1),
                        L,
                        L,
                        angle=45,
                        color=color,
        ...
        for pauli in self.box:
            ...
            if unhidden:
                if self.mode != "line":
                    ...
                    if "Y" in pauli:
                        self.ax.add_patch(
                            Circle(self.box[pauli], r, color=circle_color[1])
                        )
                    else:
                        self.ax.add_patch(
                            Circle(self.box[pauli], r, color=circle_color[0])
                        )
        ...
        if self.mode == "line":
            if bloch in ["0", "1"]:
                for other in "IXZ":
                    ...
                    for j, point in enumerate(self.points[pz]):
                        if see_if_unhidden(pz):
                            point.center = (
                                self.box[pz][0] - (prob_x - 0.5) * length,
                                self.box[pz][1] - (prob_z - 0.5) * length,
                            )
                ...
                for pauli in self.box:
                    ...
                    if pauli in ["XZ", "ZX"] + self.y_boxes * [
                        "IY",
                        "YI",
                        "XY",
                        "YX",
