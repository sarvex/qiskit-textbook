    ...
    for output in cell.outputs:
        if hasattr(output, "name") and output.name == "stderr":
            try:  # Try to identify warning type
                warning_name = re.search(r"(?<=\s)([A-Z][a-z0-9]+)+(?=:)", output.text)[
                    0
                ]
                description = re.split(warning_name, output.text, maxsplit=1)[1].strip(
                    " :"
                )
            except TypeError:
...
def run_notebook(filepath, write=False, fail_on_warning=False):
    ...
    processor.preprocess(notebook, {"metadata": {"path": filepath.parents[0]}})
    ...
    for cell in notebook.cells:
        ...
        ignore_warning_tag = (
            hasattr(cell.metadata, "tags") and "ignore-warning" in cell.metadata.tags
        )
...
if __name__ == "__main__":
    ...
    for switch in switches:
        if switch == "--fail-on-warning":
            ...
            write = True
    log = {"t0": time.time(), "total_time": 0, "total_files": 0, "broken_files": 0}
    ...
    if log["broken_files"] > 0:
        print(
            f"Found problems in {log['broken_files']}/{log['total_files']} "
            "notebooks, see output above for more info.\n"
        )
        if fail_on_warning:
            print(
                "If this test failed due to a new warning that is out of "
