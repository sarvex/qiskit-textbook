...
def lint_notebook(filepath, CI=False):
    ...
    outdir = Path(TEMP_DIR) / Path(filepath).parent.stem / Path(filepath).stem
...
def lint_markdown(md_dir, CI=False):
    ...
    vale_result = subprocess.run(
        ["vale", path, "--output", "JSON"], capture_output=True
    )
    ...
    for file, suggestions in vale_output.items():
        ...
        for s in suggestions:
            ...
            cell_msg += style("faint", f"@l{s['Line']};c{s['Span'][0]} ({s['Check']})")
            cell_msg += "\n"
